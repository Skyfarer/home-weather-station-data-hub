...
if [ -n "$LAST_UPDATE" ]; then
    # Parse the last update (format: "timestamp: temp:pressure:humidity:voltage")
    TIMESTAMP=$(echo "$LAST_UPDATE" | awk '{print $1}' | tr -d ':')
    VALUES=$(echo "$LAST_UPDATE" | awk '{print $2}')

    # Split values (temp:pressure:humidity:voltage)
    TEMP_C=$(echo "$VALUES" | cut -d':' -f1)
    PRESSURE=$(echo "$VALUES" | cut -d':' -f2)
    HUMIDITY=$(echo "$VALUES" | cut -d':' -f3)
    VOLTAGE=$(echo "$VALUES" | cut -d':' -f4)
